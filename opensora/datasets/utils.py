...
def to_pil_images(x, normalize=True, value_range=(-1, 1)):
    """
    Args:
        x (Tensor): shape [C, T, H, W]
    """
    ...
    x = list(x.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 3, 0).to("cpu", torch.uint8))
    pil_images = [Image.fromarray(img.numpy()) for img in x]
    return pil_images
