...
class VariableVideoTextDataset(VideoTextDataset):
    ...
    def getitem(self, index):
        ...
        index, num_frames, height, width = [int(val) for val in index.split("-")]  # determined by the bucket
    ...
    def __getitem__(self, index):
        try:
            ...
            data_index, num_frames, height, width = [int(val) for val in index.split("-")]  # determined by the bucket
            print(f"Error in {index}, path: {self.data.iloc[data_index]['path']}")
            traceback.print_exc()
            return None
