...
class T2IFinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size, num_patch, out_channels, d_t=None, d_s=None, pa_vdm=False):
        super().__init__()
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.linear = nn.Linear(hidden_size, num_patch * out_channels, bias=True)
    ...
    def forward(self, x, t, x_mask=None, t0=None, T=None, S=None):
        ...
        if S is None:
            S = self.d_s
        if self.pa_vdm:  # x (5, 24300, 1152), t (5, 60, 1152)
            shift, scale = (self.scale_shift_table[None, None] + t[:, :, None]).chunk(2, dim=2)  # (1, 1, 2, 1152) + (5, 60, 1, 1152) -> (5, 60, 1, 1152)
            x_t_s = rearrange(x, "B (T S) C -> B T S C", T=T, S=S)  # split T and S dimensions
            x = t2i_modulate(self.norm_final(x_t_s), shift, scale)
            x = rearrange(x, "B T S C -> B (T S) C")
        else:
            shift, scale = (self.scale_shift_table[None] + t[:, None]).chunk(2, dim=1)  # (1, 2, 1152) + (5, 1, 1152) -> (5, 2, 1152) -> (5, 1, 1152)
...
class TimestepEmbedder(nn.Module):
    ...
    def timestep_embedding(t, dim, max_period=10000):
        ...
        half = dim // 2  # 256 // 2 = 128
        freqs = torch.exp(-math.log(max_period) * torch.arange(start=0, end=half, dtype=torch.float32) / half)  # (128,)
        freqs = freqs.to(device=t.device)
        args = t[:, None].float() * freqs[None]  # (5, 128)
        embedding = torch.cat([torch.cos(args), torch.sin(args)], dim=-1)  # (5, 256)
        if dim % 2:
            embedding = torch.cat([embedding, torch.zeros_like(embedding[:, :1])], dim=-1)
        return embedding

    def forward(self, t, dtype):
        t_freq = self.timestep_embedding(t, self.frequency_embedding_size)  # (5, 256)
        if t_freq.dtype != dtype:
            t_freq = t_freq.to(dtype)
        t_emb = self.mlp(t_freq)  # (5, 1152)
        return t_emb
