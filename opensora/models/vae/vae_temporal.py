...
class VAE_Temporal(nn.Module):
    ...
    def get_latent_size(self, input_size):
        ...
        for i in range(3):
            if input_size[i] is None:
                ...
                time_padding = (
                    0
                    if (input_size[i] % self.time_downsample_factor == 0)  # time_downsample_factor = 4
                    else self.time_downsample_factor - input_size[i] % self.time_downsample_factor
                )  # 3 for 17 micro frames
                lsize = (input_size[i] + time_padding) // self.patch_size[i]  # patch size = (4, 1, 1). (17+3)//4 = 5
            else:
                lsize = input_size[i] // self.patch_size[i]
            latent_size.append(lsize)
        return latent_size  # (17, 45, 80) -> (5, 45, 80)
    ...
    def encode(self, x):
        ...
        x = pad_at_dim(x, (time_padding, 0), dim=2)  # zero-pad time dimension at the beginning to be divisible by time_downsample_factor
        encoded_feature = self.encoder(x)  # (1, 4, 4, 45, 80) -> (1, 8, 1, 45, 80), i.e. T /= 4
        moments = self.quant_conv(encoded_feature).to(x.dtype)  # same size as encoded_feature
        posterior = DiagonalGaussianDistribution(moments)  # mean and std each uses half of the parameters and has size (1, 4, 1, 45, 80)
        return posterior
