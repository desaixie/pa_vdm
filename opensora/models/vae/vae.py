...
class VideoAutoencoderKL(nn.Module):
    ...
    def encode(self, x):
        # x: (B, C, T, H, W)
        B = x.shape[0]
        x = rearrange(x, "B C T H W -> (B T) C H W")  # merge batch and time dimensions
        ...
        x = rearrange(x, "(B T) C H W -> B C T H W", B=B)  # extract batch and time dimensions
        return x
    ...
    def get_latent_size(self, input_size):
        ...
        return latent_size  # (17, 360, 640) -> (17, 45, 80), from hard-coded patch size (1, 8, 8)
...
class VideoAutoencoderPipeline(PreTrainedModel):
    ...
    def encode(self, x):
        x_z = self.spatial_vae.encode(x)  # (B=1, C=3, T=1, H=360, W=640) -> (B=1, C=4, T=1, H=45, W=80) for a image
        ...
        if self.micro_frame_size is None:
            ...
            for i in range(0, x_z.shape[2], self.micro_frame_size):
                x_z_bs = x_z[:, :, i : i + self.micro_frame_size]  # split T into a minibatch of up to 17 (hard-coded) frames
                posterior = self.temporal_vae.encode(x_z_bs)  # (1, 4, 1, 45, 80) -> (1, 4, 1, 45, 80)
                z_list.append(posterior.sample())
            z = torch.cat(z_list, dim=2)  # (1, 4, 1, 45, 80)
    ...
    def get_latent_size(self, input_size):
        if self.micro_frame_size is None or input_size[0] is None:
            ...
            sub_input_size = [self.micro_frame_size, input_size[1], input_size[2]]  # split into 17 (hard-coded) frames, (51, 360, 640) -> (17, 360, 640)
            sub_latent_size = self.temporal_vae.get_latent_size(self.spatial_vae.get_latent_size(sub_input_size))  # (17, 360, 640) -> (5, 45, 80), through spatial (1, 8, 8) then temporal (4, 1, 1)
            sub_latent_size[0] = sub_latent_size[0] * (input_size[0] // self.micro_frame_size)  # *= (51 // 17), (5, 45, 80) -> (15, 45, 80)
            remain_temporal_size = [input_size[0] % self.micro_frame_size, None, None]  # check residual frames
            ...
            return sub_latent_size  # (51, 360, 640) -> (15, 45, 80)
