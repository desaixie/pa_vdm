...
class STDiT3Block(nn.Module):
    ...
    def forward(
        ...
        B, N, C = x.shape  # 5, 24300 (S=405 * T=60), 1152
        # get the 6 adaLN-Zero parameters for t
        if self.pa_vdm:
            assert t.shape == (B, T, 6 * C)
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (  # (5, 60, 1, 1152)
                    self.scale_shift_table[None, None] + t.reshape(B, T, 6, -1)  # (1, 1, 6, 1152) + (5, 60, 6, 1152).
            ).chunk(6, dim=2)
        else:
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (  # (5, 1, 1152).
                self.scale_shift_table[None] + t.reshape(B, 6, -1)  # (1, 6, 1152) + (5, 6, 1152).
            ).chunk(6, dim=1)
        if x_mask is not None:
            ...
            if self.pa_vdm:
                raise NotImplementedError("pa vdm not supported for x_mask")

        # modulate (attention).
        if self.pa_vdm:
            x_t_s = rearrange(x, "B (T S) C -> B T S C", T=T, S=S)  # split T and S dimension
            x_m = t2i_modulate(self.norm1(x_t_s), shift_msa, scale_msa)  # (5, 60, 405, 1152) +/* (5, 60, 1, 1152)
            x_m = rearrange(x_m, "B T S C -> B (T S) C", T=T, S=S)  # merge back T and S. (5, 60, 405, 1152) -> (5, 24300, 1152)
        else:
            x_m = t2i_modulate(self.norm1(x), shift_msa, scale_msa)  # (5, 24300, 1152) +/* (5, 1, 1152)
        if x_mask is not None:
        ...
        # modulate (attention)
        if self.pa_vdm:
            x_m_t_s = rearrange(x_m, "B (T S) C -> B T S C", T=T, S=S)  # split T and S dimension (5, 24300, 1152) -> (5, 60, 405, 1152)
            x_m_s = gate_msa * x_m_t_s  # (5, 60, 1, 1152) * (5, 60, 405, 1152)
            x_m_s = rearrange(x_m_s, "B T S C -> B (T S) C", T=T, S=S)  # merge back T and S. (5, 60, 405, 1152) -> (5, 24300, 1152)
        else:
            x_m_s = gate_msa * x_m
        if x_mask is not None:
        ...
        # modulate (MLP)
        if self.pa_vdm:
            x_t_s = rearrange(x, "B (T S) C -> B T S C", T=T, S=S)
            x_m = t2i_modulate(self.norm2(x_t_s), shift_mlp, scale_mlp)
            x_m = rearrange(x_m, "B T S C -> B (T S) C", T=T, S=S)
        else:
        ...
        # modulate (MLP)
        if self.pa_vdm:
            x_m_t_s = rearrange(x_m, "B (T S) C -> B T S C", T=T, S=S)
            x_m_s = gate_mlp * x_m_t_s
            x_m_s = rearrange(x_m_s, "B T S C -> B (T S) C", T=T, S=S)
        else:
            x_m_s = gate_mlp * x_m
        if x_mask is not None:
...
class STDiT3(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # final layer
        self.final_layer = T2IFinalLayer(config.hidden_size, np.prod(self.patch_size), self.out_channels, pa_vdm=config.pa_vdm)
    ...
    def forward(self, x, timestep, y, mask=None, x_mask=None, fps=None, height=None, width=None, **kwargs):
        ...
        B = x.size(0)
        x = x.to(dtype)
        timestep = timestep.to(dtype)  # (5,). Ours: (5, 60)
        y = y.to(dtype)
        ...
        # === get timestep embed ===
        if self.pa_vdm:
            assert timestep.shape == (B, T), f'{timestep.shape} != {(B, T)}'
            timestep = timestep.reshape(-1)  # (5, 60) -> (5*60)
            t = self.t_embedder(timestep, dtype=x.dtype)  # [B*T, C] (300, 1152)
            fps = self.fps_embedder(fps.unsqueeze(1), B)  # (5, 1152)
            t = (t.reshape(B, T, -1) + fps.unsqueeze(1)).reshape(B*T, -1)  # (5, 60, 1152) + (5, 1, 1152) -> (300, 1152)
            t_mlp = self.t_block(t)  # (300, 6912), i.e. 6x1152, to be chunked into 6 variables: shift/scale/gate for MSA and MLP
            t = t.reshape(B, T, -1)
            t_mlp = t_mlp.reshape(B, T, -1)
        else:
            t = self.t_embedder(timestep, dtype=x.dtype)  # [B, C] (5, 1152)
            fps = self.fps_embedder(fps.unsqueeze(1), B)  # (5, 1152)
            t = t + fps
            t_mlp = self.t_block(t)  # (5, 6912), i.e. 6x1152, to be chunked into 6 variables: shift/scale/gate for MSA and MLP
        t0 = t0_mlp = None
        if x_mask is not None:
